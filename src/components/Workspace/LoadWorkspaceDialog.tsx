/**
 * LoadWorkspaceDialog Component
 *
 * Modal for loading saved workspace configurations.
 * Shows a list of available workspaces with options to load or delete them.
 */
...
export function LoadWorkspaceDialog({ isOpen, onClose, onLoad }: LoadWorkspaceDialogProps) {
  ...
  const handleDelete = useCallback(async (fileName: string, name: string) => {
    ...
    try {
      const result = await window.electron.workspace.delete(fileName)
      if (result.success) {
        setWorkspaces(prev => prev.filter(f => f.fileName !== fileName))
      } else {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-background border border-border rounded-lg shadow-lg w-full max-w-md mx-4 max-h-[80vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-border flex-shrink-0">
          <h2 className="text-sm font-semibold">Load Workspace</h2>
          <button
            onClick={onClose}
        ...
        {/* Content */}
        <div className="flex-1 overflow-auto p-2 widget-scroll">
          {isLoading ? (
              ...
              {workspaces.map((workspace) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      onClick={() => handleLoad(workspace.fileName)}
                    ...
                    <button
                      onClick={() => handleDelete(workspace.fileName, workspace.name)}
                      disabled={loadingFileName !== null || deletingFileName !== null}
                      className={cn(
                        'p-1 rounded',
                      ...
                      title="Delete"
                    >
                      {deletingFileName === workspace.fileName ? (
